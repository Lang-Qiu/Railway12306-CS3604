...
class RegisterController {
  async validateUsername(req, res) {
    try {
      ...
      stmt.bind([username])
      const existsInSql = stmt.step()
      stmt.free()
      const existingUser = existsInSql ? {} : await jsonDbService.findUserBy(username, 'username')
  ...
  async register(req, res) {
    try {
      ...
      const db = await dbService.getDb()
      let stmt = db.prepare('SELECT 1 FROM users WHERE username = ? LIMIT 1')
      stmt.bind([username])
      const usernameExists = stmt.step()
      stmt.free()
      if (usernameExists) return res.status(409).json({ error: '该用户名已经占用，请重新选择用户名！' })
      stmt = db.prepare('SELECT 1 FROM users WHERE id_card_type = ? AND id_card_number = ? LIMIT 1')
      stmt.bind([idCardType, idCardNumber])
      const idExists = stmt.step()
      stmt.free()
      if (idExists) return res.status(409).json({ error: '该证件号码已经被注册过，请确认是否您本人注册，"是"请使用原账号登录，"不是"请通过铁路12306App办理抢注或持该证件到就近的办理客运业务的铁路车站办理被抢注处理，完成后即可继续注册，或致电12306客服咨询。' })
