...
class OrderService {
  ...
  async getOrder(orderId, userId) {
      const order = db.get('SELECT * FROM orders WHERE id = ? AND user_id = ?', [orderId, userId]);
      if (!order) return null;
      
      const items = db.all(`
        SELECT oi.*, p.name as passenger_name, p.id_card_number, st.name as seat_type_name
        FROM order_items oi
